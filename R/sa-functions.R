...
function(agent, date_DALY, exp) {
  ...
  ## define contribution
  file <-
    paste0("DALY_", agent, "_", crpop_2015$ISO3[1], "-", date_DALY, ".RData")
  load(file)
  ...
  for (i in seq(NumCountries)) {
    ...
    ## which subregion?
    r <- as.character(countryRegion_2015$WHOSubregion[i])
    ...
    ## save DALYrun object
    file_new <-
      paste0("FB_DALY_", agent, "_", crpop_2015$ISO3[i], "-", today, ".RData")
